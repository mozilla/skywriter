...
// This script appears at the end of SkywriterEmbeddedMain and is responsible
...
skywriter.initialized = new Promise();
...
skywriter.useSkywriter = function(element, options) {
    var util = skywriter.tiki.require('skywriter:util/util');
    ...
    skywriter.tiki.require.ensurePackage("::appconfig", function() {
        var appconfig = skywriter.tiki.require("appconfig");
        ...
        if (element.type == 'textarea') {
            ...
            // This will hold the Skywriter editor.
            var container = document.createElement('div');

            // To put Skywriter in the place of the textarea, we have to copy a
...
$(document).ready(function() {
    // Skywriter is now ready to use.
    skywriter.loaded.resolve();

    // Holds the lauch promises of all launched Skywriters.
    var launchSkywriterPromises = [];

    var nodes = document.querySelectorAll(".skywriter");
    for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        var options = node.getAttribute('data-skywriteroptions') || '{}';
        var pr = skywriter.useSkywriter(node, JSON.parse(options));
        pr.then(function(env) {
            node.skywriter = env;
        }, function(error) {
            throw new Error('Launch failed: ' + error);
        });
        launchSkywriterPromises.push(pr);
    }
    ...
    Promise.group(launchSkywriterPromises).then(function() {
        ...
        if (window.onSkywriterLoad) {
          window.onSkywriterLoad();
        }
    }, function(err) {
        skywriter.initialized.reject('At least one Skywriter failed to launch!' + err);
    });
});
